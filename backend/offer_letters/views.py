...
def _generate_signature_id(prefix='OFR'):
    now = timezone.now()
...
def _collect_esig_metadata(request, sig_prefix='OFR'):
    tz = ''
        ...
        if doc_type == 'signed':
            if offer.signature_data or offer.company_signature_data:
                pdf_buf = generate_offer_letter_pdf(
                    ...
                    company_signature=getattr(offer, 'company_signature_data', None),
                    company_signer_name=getattr(offer, 'company_signer_name', ''),
                    company_signer_position=getattr(offer, 'company_signer_position', ''),
                    company_signed_date=getattr(offer, 'company_signed_at', None),
                    employee_esignature_metadata=getattr(offer, 'esignature_metadata', None),
                    company_esignature_metadata=getattr(offer, 'company_esignature_metadata', None),
                )
                if not pdf_buf:
