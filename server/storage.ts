...
export interface IStorage {
  ...
  updateUserFullName(userId: number, fullName: string): Promise<void>;
...
export class DatabaseStorage implements IStorage {
  ...
  async deleteDocument(id: number): Promise<void> {
    await db.delete(agreementDocuments).where(eq(agreementDocuments.id, id));
  }
  ...
  async updateStudentProvider(id: number, data: any) {
    const [updated] = await db.update(studentProviders).set(data).where(eq(studentProviders.id, id)).returning();
    return updated;
  }
  ...
  async updateUserFullName(userId: number, fullName: string): Promise<void> {
    await db.update(users).set({ fullName, updatedAt: new Date() }).where(eq(users.id, userId));
  }
