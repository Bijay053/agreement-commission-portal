...
- **Multi-Provider Students with Per-Provider Commission**: Each student can have multiple providers. Primary provider on `commission_students`, additional providers in `student_providers`. Each provider has its own commission config (`commissionRatePct`, `gstApplicable`, `gstRatePct`, `scholarshipType`, `scholarshipValue`, `status`, `notes`, `courseLevel`, `courseName`). Commission entries link to specific providers via `studentProviderId` FK. The Master table shows per-provider rows with independent editable fields for commission rate, GST, scholarship, status, and notes. Term tabs show separate entry rows per provider, each with its own "+ Add Entry" button. Cascade hide logic: students with Withdrawn/Complete status are completely removed (not blurred) from all later intake tables. Start intake visibility: students only appear from their start intake onward. Provider deletion has confirmation dialog with stronger warning for last provider. Duplicate prevention checks (provider + course + courseLevel + intake) combination. Add Provider popup collects full academic fields (Course Name, Course Level, Duration, Intake, Country, Notes).
- **Intake Normalization**: Shared utility (`shared/intake-utils.ts`) normalizes intake formats (t12025, T12026, t1 2026, T1-2026 → "T1 2025"). Used consistently across term tables, dashboard, and visibility logic. Functions: `parseIntake`, `normalizeIntake`, `compareIntake`, `intakeSortKey`, `isFinalStatus`.
- **Master Sheet Audit Logging**: All Master Sheet edits (student and provider fields) are logged to `audit_logs` table with userId, fieldName, oldValue, newValue, entityType, and entityId. Permission `commission_tracker.master.edit` controls Master tab editing access.
